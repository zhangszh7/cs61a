class VendingMachine:
    ...
    def vend(self):
        if self.stock > 0:
            diff = abs(self.balance - self.price)
            if self.balance == self.price:
                ...
                self.balance = 0
                self.stock -= 1
                return 'Here is your {} and ${} change.'.format(self.name, diff)
            else:
...
def is_bst(t):
    ...
    if branch_amount == 1:
        b = t.branches[0]
        return not (t.label < bst_max(b) and t.label >= bst_min(b))
    elif branch_amount == 2:
        left = t.branches[0]
        right = t.branches[1]
        return (t.label >= bst_max(left) and t.label < bst_min(right))
...
def preorder(t):
    ...
    for b in t.branches:
        print_list += preorder(b) 
    
    return [t.label] + print_list

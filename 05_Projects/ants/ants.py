    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                assert not self.can_contain(place.ant), 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while place.is_hive() != True :
            if place.bees != [] and self.min_range <= idx <= self.max_range:
...
class BodyguardAnt(ContainerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    def __init__(self, *args, **kwargs):
        super().__init__(health=2, *args, **kwargs)
    # BEGIN Problem 8
    implemented = True   # Change to True to view in the GUI
    # END Problem 8
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if self.is_queen == False:
            super().reduce_health(self.health)
        else:
            place = self.place.exit
            while place != None:
                if place.ant != None and place.ant.has_buff == False:
                    place.ant.has_buff = True
                    place.ant.buff()

                if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant != None and place.ant.contained_ant.has_buff == False:
                    place.ant.contained_ant.has_buff = True
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health(amount)
        if self.health <= amount and self.is_queen == True:
            bees_win()
...
class Bee(Insect):
    ...
    def slow(self, length):
        ...
        self.apply_status(slow_action, old_action, length)
        # END Problem Optional 2
    ...
    def scare(self, length):
        ...
        if self.scared == False:    
            self.apply_status(scare_action, old_action, length)
        # END Problem Optional 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while place is not beehive:
            if place.ant != self and place.ant != None:
                insects[place.ant] = distance
            if isinstance(place.ant, ContainerAnt) and place.ant.contained_ant != None:
                if place.ant.contained_ant != self:
                    insects[place.ant.contained_ant] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 3
        damage = self.damage - self.insects_shot * 0.0625 - distance * 0.25

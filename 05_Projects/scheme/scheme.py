...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        if rest == nil:
            return scheme_eval(current, env, True)
        else:
            scheme_eval(current, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        child = Frame(self)
        while formals != nil:
            symbol, formals = formals.first, formals.rest
            value, vals = vals.first, vals.rest
            child.define(symbol, value)
        return child
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        def tolist(pair):
            if pair == nil:
               return []
            return [pair.first] + tolist(pair.rest)
        
        python_args += tolist(args)

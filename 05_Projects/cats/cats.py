...
def pawssible_patches(start, goal, limit):
    ...
    if limit - abs(len(start) - len(goal)) < 0 or start == "" or goal == "": 
        # BEGIN
        "*** YOUR CODE HERE ***"
        return abs(len(start) - len(goal))       
        # END
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        player_i = times_per_player[i]
        times_per_player[i] = [player_i[j+1] - player_i[j] for j in range(len(player_i) - 1)]
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_word = [ [] for _ in player_indices ]
    for i in word_indices:
            times = [time(game, j, i) for j in player_indices]
            for z in player_indices:
                if time(game, z, i) == min(times):
                    fastest_word[z] += [word_at(game, i)]
                    break
    return fastest_word

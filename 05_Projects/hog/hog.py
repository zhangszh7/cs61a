...
def swine_align(player_score, opponent_score):
    ...
    # evaluate the GCD
    num, den = max(player_score, opponent_score), min(player_score, opponent_score)
    while num % den:
        num, den = den, num % den
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if 0 < (opponent_score - player_score)  and (opponent_score - player_score) < 3:
        return True
    return False
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def turn(strategy, player_score, opponent_score):
            player_score += take_turn(strategy(player_score, opponent_score), opponent_score, dice) 
            return player_score, extra_turn(player_score, opponent_score)  


    while score0 < goal and score1 < goal:
        # play a turn
        if who == 0: 
            score0, next_turn = turn(strategy0, score0, score1)
        else:
            score1, next_turn = turn(strategy1, score1, score0)
        
        say = say(score0, score1)        

        # next turn
        who = who if next_turn else other(who)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    return 0 if extra_turn(score + free_bacon(opponent_score), opponent_score) \
        else bacon_strategy(score, opponent_score, cutoff, num_rolls)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return extra_turn_strategy(score, opponent_score, 10, 6)
    # END PROBLEM 12

...
def prune_min(t):
    ...
    sub_t = t.branches[0] if t.branches[0].label < t.branches[1].label else t.branches[1]
    prune_min(sub_t)
    t.branches = [sub_t]
...
def num_splits(s, d):
    ...
    num = 0
    equal = 0
    def split(s, first, repeat):
        nonlocal num, equal
        if first - sum(s) == 0 and not repeat:
            equal += 1
        elif  first - sum(s) > 0 and first - sum(s) <= d and not repeat:
            num += 1
        ...
        split(rest, first - s[0], 1)
        split(rest, first + s[0], 0)
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        ...
        elif check.account != self.holder:
            print("The police have been notified.")
        else:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            match, cost = skeleton[skeleton_idx] + match, cost
            possibilities.append((match, cost))
        # Insert
        match, cost = helper_align(skeleton_idx, code_idx+1)
        match, cost = "+[" + code[code_idx] + "]" + match, cost+1
        possibilities.append((match, cost))
        # Delete
        match, cost = helper_align(skeleton_idx+1, code_idx)
        match, cost = "-[" + skeleton[skeleton_idx] + "]" + match, cost+1
        possibilities.append((match, cost))
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldr(lst, lambda lst, e: Link(e, lst) if pred(e) else lst, Link.empty)
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda lst, e: Link(e, lst), Link.empty)

...
def subseqs(s):
    ...
    if not s:
        ...
        subseq = subseqs(s[1:])
        return insert_into_all(s[0], subseq) + subseq
...
def shuffle(cards):
    ...
    half = [cards[j] for j in range(len(cards) // 2, len(cards)) ]
    shuffled = []
...
def insert(link, value, index):
    ...
    """
    if index == 0 and link:
        link.rest = Link(0, link.rest)
        link.rest.first = link.first
        link.first = value
    elif index > 0 and link:
...
def prune_small(t, n):
    ...
    while len(t.branches) > n:
        largest = max(t.branches, key=lambda self: self.label)
